<template>
  ...
  >
    ...
    <header class="header-table vs-table--header">
      ...
      <div>
        <div v-if="search" class="con-input-search vs-table--search pb-0">
          <input
            v-model="searchx"
        ...
        <span
          class="text-danger text-sm w-full text-center ml-3"
          v-if="search && searchErrorMessage"
        >
          {{ searchErrorMessage }}
        </span>
      </div>
    ...
    <div class="con-tablex vs-table--content">
      ...
      <div :style="styleConTbody" class="vs-con-tbody vs-table--tbody ">
        <table ref="table" class="vs-table vs-table--tbody-table" :class="tableClass">
    ...
    searchx() {
      if (this.sst) {
        if(this.searchRegex) {
          ...
          try {
            new RegExp(this.searchx);
            this.$emit("search", this.searchx);
          } catch(e) {
            this.searchErrorMessage = 'Invalid Regex Expression';
          }
        } else {
          this.$emit("search", this.searchx);
        }
