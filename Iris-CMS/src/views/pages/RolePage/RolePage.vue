        ...
        <tbody>
          ...
          >
            ...
            <vs-td class="whitespace-no-wrap">
              <div class="flex">
                <vx-tooltip text="Edit" v-if="$can('role', 'update')">
                  <feather-icon
                    icon="EditIcon"
                ...
                <vx-tooltip text="Delete" v-if="$can('role', 'delete')">
                  <feather-icon
                    icon="TrashIcon"
...
export default {
  ...
  methods: {
    ...
    addNewData() {
      this.$router.push("/dashboard/role-create");
    },
    confirmDelete(tr) {
      this.$vs.dialog({
        type: "confirm",
        color: "danger",
        title: `Confirm`,
        text: `You are about to DELETE '${tr.role}' Role. this is irreversible (cannot be undone).`,
    ...
    async deleteData(tr, notify = true) {
      ...
      if (tr.id == "super_admin") {
        this.$vs.notify({
          position: "bottom-center",
          color: "danger",
          text: "super admin role is immortal and may not deleted!",
          title: "Cannot Delete Super Admin!",
        });
        return;
      }
      let res = await this.$store
        .dispatch("role/deleteRole", tr.id)
        .catch(handleFirestoreReject.bind(this));
      if (typeof res == "object" && res.color && notify) this.$vs.notify(res);
    },
    editData(tr) {
      this.$store.commit("role/setUpdateData", tr);
      this.$router.push("/dashboard/role-update");
    },
  },
};
</script>
